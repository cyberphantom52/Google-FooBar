...
injection system for the LAMBCHOP doomsday device. It's a great chance for you to get a closer 
look at the LAMBCHOP -- and maybe sneak in a bit of sabotage while you're at it -- so you took 
the job gladly. 
...
# Find the number of trailing zeros in a binary representation of a number
...
def solution(n):
    ...
    while n > 1:
        ...
        zeroes = trailing_zeros(bin(n))

        if zeroes > 0:
            # Split the fuel pellets in half as many times as there are trailing zeros
            n >>= zeroes
            min_operations += zeroes
        else:
            min_operations += 1
            if trailing_zeros(bin(n + 1)) > trailing_zeros(bin(n - 1)):
                # Add a fuel pellet
                n += 1
            else:
                # Remove a fuel pellet
                n -= 1
